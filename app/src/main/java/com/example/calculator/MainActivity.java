...
public class MainActivity extends AppCompatActivity {
    ...
    b9,b0,bac,bc,bp,bsqrt,bplus,
    bminus,bdiv,bmod,bequal,bdot,
    bbrac1,bbrac2,bsin,bcos,btan,
    bsquare,bpi,bfact,blog,bln,binv;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //onclick listeners (fonctions pour manipuler le texte de calculatrice)
    ...
    int factorial(int n)
    {
        ...
        // trouver factorial par recursivite
    ...
    public static double eval(final String str) {
        return new Object() {
            ...
            //        | nombre | nomFonction facteur | facteur `^` facteur

            double parseExpression() {
                double x = parseTerm();
                for (;;) {
                    if      (manger('+')) x += parseTerm(); // addition
                    else if (manger('-')) x -= parseTerm(); // soustraction
                    else return x;
                }
            }

            double parseTerm() {
                double x = parseFacteur();
                for (;;) {
                    if      (manger('*')) x *= parseFacteur(); // multiplication
                    else if (manger('/')) x /= parseFacteur(); // la division
                    else return x;
                }
            }

            double parseFacteur() {
                if (manger('+')) return parseFacteur(); //  plus
                if (manger('-')) return -parseFacteur(); //  moins

                double x;
                int startPos = this.pos;
                if (manger('(')) { // parentheses
                    x = parseExpression();
                    manger(')');
                    ...
                    // La converion de degre vers le radian pour le calcul trigo
                ...
                if (manger('^')) x = Math.pow(x, parseFacteur()); // pour calculer par l`exponent
